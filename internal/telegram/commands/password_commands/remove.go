...
func RemovePassword(b *bot.Bot, service *service.Service, password module.Password, wait chan tgbotapi.Message, cancel waiting.Cancel) bot.Command {
	return func(ctx context.Context, update tgbotapi.Update) error {
		...
		case <-cancel.Canceled():
			return nil
		case <-ctx.Done():
			return b.Send(update.FromChat().ID, update.Message.MessageID, "I'm sorry, removing password interrupted")
		case answer := <-wait:
			ok, err := service.CheckUserPassword(ctx, update.SentFrom().ID, answer.Text)
			if err != nil {
				return b.Send(update.FromChat().ID, update.Message.MessageID, fmt.Sprintf("Removing password failed with error: %v", err))
			} else if !ok {
				return b.Send(update.FromChat().ID, update.Message.MessageID, "Removing password failed: wrong password")
			}
		}
		...
		return b.Send(update.FromChat().ID, update.Message.MessageID, "Removed password")
